(() => {
  ...
  function drawEq() {
    ...
    // Перерисовка EQ удаляет все SVG-узлы, поэтому старая FFT-линия
    // должна быть инвалидирована, иначе animation loop будет обновлять
    // отсоединённый DOM-элемент после перетаскивания точки.
    spectrumPath = null;
    svg.replaceChildren();
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    function animateSpectrum() {
      ...
      if (!spectrumPath || !spectrumPath.isConnected) {
